...
const CORE_RESET_SECTION_ID = '731286';
const CORE_RESET_SECTION_NAME = 'SYSTEM_CORE_RESET';
const CORE_RESET_SYMBOL_ID = 'SYM_SYSTEM_121';
...
const SECTION_DOC = {
  id: CORE_RESET_SECTION_ID,
  name: CORE_RESET_SECTION_NAME,
  category: 'SYSTEM',
  description: 'Delete and rebuild core bootstrap registry entities in one protected workflow.',
  homeURL: '/systemSettings/core-reset',
  message: '',
  inactiveMessage: '',
  active: true,
...
async function upsertCoreResetSection(sections) {
  const existing =
    await sections.findOne({ id: CORE_RESET_SECTION_ID })
    || await sections.findOne({ name: { $regex: new RegExp(`^${escapeRegex(CORE_RESET_SECTION_NAME)}$`, 'i') } });
  ...
  if (!existing) {
    ...
    console.log(`Inserted section ${CORE_RESET_SECTION_NAME} (${next.id}).`);
    return next;
  }

  await sections.updateOne({ _id: existing._id }, { $set: next });
  console.log(`Updated section ${CORE_RESET_SECTION_NAME} (${next.id}).`);
...
async function linkUnderNavigatorParent(sections, childSection) {
  ...
  console.log(`Linked ${CORE_RESET_SECTION_NAME} (${childId}) under ${NAV_PARENT_SECTION.name}.`);
}

function buildSymbolDoc(sectionId) {
  return {
    id: CORE_RESET_SYMBOL_ID,
    name: CORE_RESET_SECTION_NAME,
    type: 'class',
    value: 'bi bi-arrow-clockwise',
    tags: [
      CORE_RESET_SECTION_NAME,
      'CORE_RESET',
      'SYSTEM_CORE_RESET',
      String(sectionId || CORE_RESET_SECTION_ID)
    ],
    orgId: 'SYSTEM'
  };
}

async function upsertCoreResetSymbol(symbols, sectionId) {
  const symbolDoc = buildSymbolDoc(sectionId);
  const existing =
    await symbols.findOne({ id: symbolDoc.id })
    || await symbols.findOne({
      name: { $regex: new RegExp(`^${escapeRegex(CORE_RESET_SECTION_NAME)}$`, 'i') },
  ...
  if (!existing) {
    ...
    console.log(`Inserted symbol ${CORE_RESET_SECTION_NAME} (${next.id}).`);
    return next;
  }

  await symbols.updateOne({ _id: existing._id }, { $set: next });
  console.log(`Updated symbol ${CORE_RESET_SECTION_NAME} (${next.id}).`);
  return { ...existing, ...next };
}
...
async function main() {
  ...
  try {
    ...
    const section = await upsertCoreResetSection(db.collection('sections'));
    await linkUnderNavigatorParent(db.collection('sections'), section);
    await upsertCoreResetSymbol(db.collection('symbols'), section.id);
    console.log('System core reset section/symbol seed complete.');
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`System core reset section/symbol seed failed: ${error.message}`);
  process.exit(1);
});
