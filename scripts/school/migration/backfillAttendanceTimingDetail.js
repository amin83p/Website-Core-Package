...
} = require('../../../packages/school/MVC/services/school/sessionAttendanceTimingDetailService');
...
function backfillRosterRow(row = {}) {
  return backfillRosterTimingDetailFromTotals(row);
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  console.log('[BackfillAttendanceTimingDetail] Completed.');
...
if (require.main === module) {
  main()
    ...
    .catch(async (error) => {
      await disconnectMongo().catch(() => {});
      console.error(`[BackfillAttendanceTimingDetail] Failed: ${error.message}`);
      process.exit(1);
    });
}
