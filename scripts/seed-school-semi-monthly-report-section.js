...
const SECTION = {
  id: '446108',
  name: 'SCHOOL_SEMI_MONTHLY_REPORT',
  description: 'Student-centric semi-monthly report listing enrolled classes and matching report instances for a selected date range.',
  homeURL: '/school/reports/semi-monthly',
  symbolId: 'SYM_SYSTEM_136',
...
async function main() {
  ...
  try {
    ...
    console.log('School Semi-Monthly Report section and symbol seeded successfully.');
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`Semi-monthly report seed failed: ${error.message}`);
  process.exit(1);
});
