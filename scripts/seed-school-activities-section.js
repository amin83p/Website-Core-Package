...
const { seedSchoolActivitiesSection } = require('../packages/school/scripts/maintenance/seedSchoolActivitiesSection');
...
async function main() {
  ...
  try {
    ...
    console.log(`School Activities section seed complete (${result.section.action}).`);
    console.log(result.parent.message);
    console.log(`Symbol ${result.symbol.action}; SCHOOL_STAFF profiles updated: ${result.access.updatedProfiles}.`);
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`School Activities section seed failed: ${error.message}`);
  process.exit(1);
});
