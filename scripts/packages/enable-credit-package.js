...
const MANIFEST_PATH = path.join(ROOT_DIR, 'packages', 'credit', 'package.manifest.json');
const SCRIPT_ID = 'scripts/packages/enable-credit-package.js';
...
function printHumanReport(report = {}) {
  ...
  console.log(`[${mode}] Credit package registry activation`);
...
async function runEnableCreditPackage(argv = [], runtimeOptions = {}) {
  const options = parseArgs(Array.isArray(argv) ? argv : []);
  const emit = runtimeOptions.emit !== false;

  if (options.help) {
    if (emit) {
      console.log('Usage: node scripts/packages/enable-credit-package.js [--apply] [--disable|--remove] [--json]');
  ...
  const manifest = loadCreditManifest();
...
async function main() {
  await runEnableCreditPackage(process.argv.slice(2), { emit: true });
}
