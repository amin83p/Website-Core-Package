...
const SECTION_HOME_URL = '/school/library/books';
const SYMBOL_ID = 'SYM_SCHOOL_BOOKS_001';
const PARENT_SECTION = { id: '446110', name: 'SCHOOL_LIBRARY' };
const SAMPLE_SECTION_ID = '445584';
...
async function linkUnderParent(sections) {
  ...
  const next = [...refs];
  const anchorIdx = next.findIndex((row) => String(row?.id || row || '').trim() === SAMPLE_SECTION_ID);
...
async function main() {
  ...
  try {
    ...
    console.log('School Books section/symbol seed complete.');
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`School Books section/symbol seed failed: ${error.message}`);
  process.exit(1);
});
