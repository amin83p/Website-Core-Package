...
async function main() {
  ...
  try {
    ...
    const child = await upsertSectionByName(sections, PTE_QUESTIONS_BANK_SEED);
    const childId = String(child.id || '').trim();
    if (!childId) throw new Error('Failed to resolve child id for PTE_QUESTIONS_BANK.');
    ...
    if (!pteRoot) {
      ...
      console.log(`Ensured ${PTE_QUESTIONS_BANK_SEED.name} (${childId}) is linked under ${PTE_ROOT_REF.name}.`);
    }
    ...
    if (ptePeople) {
      ...
      if (filteredRefs.length !== repairedRefs.length) {
        console.log(`Removed ${PTE_QUESTIONS_BANK_SEED.name} (${childId}) from ${PTE_PEOPLE_REF.name}.`);
      } else {
        console.log(`${PTE_PEOPLE_REF.name} does not reference ${PTE_QUESTIONS_BANK_SEED.name}.`);
      }
    }

    console.log('PTE Questions Bank section seed complete.');
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`PTE Questions Bank section seed failed: ${error.message}`);
  process.exit(1);
});
