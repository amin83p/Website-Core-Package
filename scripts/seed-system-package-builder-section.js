...
const NAV_PARENT_SECTION = {
  id: '883303',
  name: 'SYSTEM_SETTING'
};

const PACKAGE_BUILDER_SECTION_ID = '731287';
const PACKAGE_BUILDER_SECTION_NAME = 'SYSTEM_PACKAGE_BUILDER';
const PACKAGE_BUILDER_SYMBOL_ID = 'SYM_SYSTEM_122';
...
const SECTION_DOC = {
  id: PACKAGE_BUILDER_SECTION_ID,
  name: PACKAGE_BUILDER_SECTION_NAME,
  category: 'SYSTEM',
  description: 'Build signed install packages with package-owned data and upload files.',
  homeURL: '/systemSettings/package-builder',
  message: '',
  inactiveMessage: '',
  active: true,
...
async function upsertPackageBuilderSection(sections) {
  const existing =
    await sections.findOne({ id: PACKAGE_BUILDER_SECTION_ID })
    || await sections.findOne({ name: { $regex: new RegExp(`^${escapeRegex(PACKAGE_BUILDER_SECTION_NAME)}$`, 'i') } });
  const next = {
    ...SECTION_DOC,
  ...
  if (!existing) {
    ...
    console.log(`Inserted section ${PACKAGE_BUILDER_SECTION_NAME} (${next.id}).`);
    return next;
  }
  await sections.updateOne({ _id: existing._id }, { $set: next });
  console.log(`Updated section ${PACKAGE_BUILDER_SECTION_NAME} (${next.id}).`);
  return { ...existing, ...next };
}

async function linkUnderNavigatorParent(sections, childSection) {
  const parent =
    await sections.findOne({ id: NAV_PARENT_SECTION.id })
    || await sections.findOne({ name: { $regex: new RegExp(`^${escapeRegex(NAV_PARENT_SECTION.name)}$`, 'i') } });
  if (!parent) {
    console.warn(`WARNING: ${NAV_PARENT_SECTION.name} was not found. Add subsection manually: { id: "${childSection.id}" }`);
    return;
  }
  ...
  console.log(`Linked ${PACKAGE_BUILDER_SECTION_NAME} (${childId}) under ${NAV_PARENT_SECTION.name}.`);
}

function buildSymbolDoc(sectionId) {
  return {
    id: PACKAGE_BUILDER_SYMBOL_ID,
    name: PACKAGE_BUILDER_SECTION_NAME,
    type: 'class',
    value: 'bi bi-box-seam',
    tags: [
      PACKAGE_BUILDER_SECTION_NAME,
      'SYSTEM_PACKAGE_BUILDER',
      'PACKAGE_BUILDER',
      String(sectionId || PACKAGE_BUILDER_SECTION_ID)
    ],
    orgId: 'SYSTEM'
  };
}

async function upsertPackageBuilderSymbol(symbols, sectionId) {
  const symbolDoc = buildSymbolDoc(sectionId);
  const existing =
    await symbols.findOne({ id: symbolDoc.id })
    || await symbols.findOne({
      name: { $regex: new RegExp(`^${escapeRegex(PACKAGE_BUILDER_SECTION_NAME)}$`, 'i') },
  ...
  if (!existing) {
    ...
    console.log(`Inserted symbol ${PACKAGE_BUILDER_SECTION_NAME} (${next.id}).`);
    return next;
  }
  await symbols.updateOne({ _id: existing._id }, { $set: next });
  console.log(`Updated symbol ${PACKAGE_BUILDER_SECTION_NAME} (${next.id}).`);
  return { ...existing, ...next };
}
...
async function main() {
  ...
  try {
    ...
    const section = await upsertPackageBuilderSection(db.collection('sections'));
    await linkUnderNavigatorParent(db.collection('sections'), section);
    await upsertPackageBuilderSymbol(db.collection('symbols'), section.id);
    console.log('System package builder section/symbol seed complete.');
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`System package builder section/symbol seed failed: ${error.message}`);
  process.exit(1);
});
