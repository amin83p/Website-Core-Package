...
const SECTION_ID = '445584';
const SECTION_NAME = 'SCHOOL_SKILLS';
const SECTION_HOME_URL = '/school/skills';
const SYMBOL_ID = 'SYM_SCHOOL_SKILLS_001';
const PARENT_SECTION = { id: '139382', name: 'SCHOOL_ACADEMIA' };
const SAMPLE_SECTION_ID = '445580';
...
function buildSectionDoc() {
  return {
    ...
    name: SECTION_NAME,
    category: 'SCHOOL',
    description: 'Manage the organization skill catalog used by class definitions, sessions, and teaching outlines.',
    active: true,
    trackState: true,
...
async function linkUnderParent(sections) {
  ...
  const next = [...refs];
  const anchorIdx = next.findIndex((row) => String(row?.id || row || '').trim() === '445569');
...
async function grantAccessFromSampleSection(accesses) {
  const profiles = await accesses.find({
    'sections.sectionId': SAMPLE_SECTION_ID
  }).toArray();
  let updated = 0;
  for (const profile of profiles) {
    ...
    const sampleGrant = (Array.isArray(profile.sections) ? profile.sections : [])
      .find((row) => String(row?.sectionId || '') === SAMPLE_SECTION_ID);
  ...
  if (!updated) {
    console.warn(`WARNING: No access profiles with ${SAMPLE_SECTION_ID} found. Grant ${SECTION_ID} manually.`);
  }
}
...
async function main() {
  ...
  try {
    ...
    await grantAccessFromSampleSection(db.collection('accesses'));
    console.log('School Skills section/symbol seed complete.');
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`School Skills section/symbol seed failed: ${error.message}`);
  process.exit(1);
});
