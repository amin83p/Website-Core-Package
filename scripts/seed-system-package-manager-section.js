...
const PACKAGE_MANAGER_SECTION_ID = '731285';
const PACKAGE_MANAGER_SECTION_NAME = 'SYSTEM_PACKAGE_MANAGER';
const PACKAGE_MANAGER_SYMBOL_ID = 'SYM_SYSTEM_120';
...
const SECTION_DOC = {
  id: PACKAGE_MANAGER_SECTION_ID,
  name: PACKAGE_MANAGER_SECTION_NAME,
  category: 'SYSTEM',
  description: 'Install, enable, pause, remove, and sync package lifecycle operations.',
  homeURL: '/systemSettings/packages',
  message: '',
  inactiveMessage: '',
  active: true,
...
async function upsertPackageManagerSection(sections) {
  const existing =
    await sections.findOne({ id: PACKAGE_MANAGER_SECTION_ID })
    || await sections.findOne({ name: { $regex: new RegExp(`^${escapeRegex(PACKAGE_MANAGER_SECTION_NAME)}$`, 'i') } });
  ...
  if (!existing) {
    ...
    console.log(`Inserted section ${PACKAGE_MANAGER_SECTION_NAME} (${next.id}).`);
    return next;
  }

  await sections.updateOne({ _id: existing._id }, { $set: next });
  console.log(`Updated section ${PACKAGE_MANAGER_SECTION_NAME} (${next.id}).`);
...
async function linkUnderNavigatorParent(sections, childSection) {
  ...
  if (!parent) {
    console.warn(
      `WARNING: ${NAV_PARENT_SECTION.name} was not found. Add subsection manually: { id: "${childSection.id}" }`
    );
    return;
  }
  ...
  console.log(`Linked ${PACKAGE_MANAGER_SECTION_NAME} (${childId}) under ${NAV_PARENT_SECTION.name}.`);
}

function buildSymbolDoc(sectionId) {
  return {
    id: PACKAGE_MANAGER_SYMBOL_ID,
    name: PACKAGE_MANAGER_SECTION_NAME,
    type: 'class',
    value: 'bi bi-boxes',
    tags: [
      PACKAGE_MANAGER_SECTION_NAME,
      'SYSTEM_PACKAGE',
      'PACKAGE_MANAGER',
      String(sectionId || PACKAGE_MANAGER_SECTION_ID)
    ],
    orgId: 'SYSTEM'
  };
}

async function upsertPackageManagerSymbol(symbols, sectionId) {
  const symbolDoc = buildSymbolDoc(sectionId);
  const existing =
    await symbols.findOne({ id: symbolDoc.id })
    || await symbols.findOne({
      name: { $regex: new RegExp(`^${escapeRegex(PACKAGE_MANAGER_SECTION_NAME)}$`, 'i') },
  ...
  if (!existing) {
    ...
    console.log(`Inserted symbol ${PACKAGE_MANAGER_SECTION_NAME} (${next.id}).`);
    return next;
  }

  await symbols.updateOne({ _id: existing._id }, { $set: next });
  console.log(`Updated symbol ${PACKAGE_MANAGER_SECTION_NAME} (${next.id}).`);
  return { ...existing, ...next };
}
...
async function main() {
  ...
  try {
    ...
    const section = await upsertPackageManagerSection(db.collection('sections'));
    await linkUnderNavigatorParent(db.collection('sections'), section);
    await upsertPackageManagerSymbol(db.collection('symbols'), section.id);
    console.log('System package manager section/symbol seed complete.');
  } finally {
    await client.close();
  }
}
...
main().catch((error) => {
  console.error(`System package manager section/symbol seed failed: ${error.message}`);
  process.exit(1);
});
