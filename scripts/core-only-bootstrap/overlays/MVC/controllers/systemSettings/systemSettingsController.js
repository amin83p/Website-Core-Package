...
function buildPackageManagerOptions(req = {}) {
  ...
  const installMethod = cleanFormText(req.body?.installMethod || req.query?.installMethod || '', 40).toLowerCase();
  const localManifestPath = cleanFormText(req.body?.localManifestPath || req.query?.localManifestPath || '', 1600);
  const manifestPath = cleanFormText(req.body?.manifestPath || req.query?.manifestPath || '', 1600);
  const resolvedSelectionRoot = resolvePackageRootFromLocalManifestPath(localManifestPath || manifestPath);
  return {
    backendMode: runtimeBackend?.mode || '',
    packageRootDir: (() => {
      const explicitPackageRoot = cleanFormText(req.body?.packageRootDir || req.query?.packageRootDir || '', 1200);
      if ((installMethod === 'local' || installMethod === 'path') && resolvedSelectionRoot) return resolvedSelectionRoot;
      if (explicitPackageRoot) return explicitPackageRoot;
      if (resolvedSelectionRoot) return resolvedSelectionRoot;
      return '';
    })(),
...
function resolvePackageRootFromLocalManifestPath(localManifestPath = '') {
  ...
  const packageDirectory = normalized.slice(0, markerIndex);
