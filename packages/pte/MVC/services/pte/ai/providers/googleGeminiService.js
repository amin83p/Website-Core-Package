...
async function fetchModelRows(credentials = {}, options = {}) {
  ...
  const useSharedCache = !s(credentials.apiKey);
...
async function resolveDefaultModelInternal(preferredModelId = null, credentials = {}, options = {}) {
  ...
  const canUseCache = !s(credentials.apiKey);
  if (canUseCache && !forceRefresh && cachedDefaultModelId) return cachedDefaultModelId;
  ...
  if (canUseCache) cachedDefaultModelId = resolved;
  return resolved;
}
...
function normalizeMessages(messages = []) {
  ...
  if (!chatMessages.length) throw new Error('At least one user/assistant message is required after system messages.');
...
function isModelNotFoundError(error) {
  ...
  return text.includes('404') || text.includes('model not found') || text.includes('not found');
}
...
function buildNoCacheHeaders(options = {}) {
...
async function sendMessage(messages, options = {}) {
  ...
  const requestHeaders = buildNoCacheHeaders({ disableCache });
  const modelRequestOptions = requestHeaders ? { customHeaders: requestHeaders } : undefined;

  let modelToUse = await resolveDefaultModelInternal(explicitModelId, credentials, {
    forceRefresh: disableCache,
    ignoreConfigured: false
  });

  const executeRequest = async (activeModelId, retriedFromModel = null) => {
    const client = getClient(credentials);
    const model = client.getGenerativeModel({ model: activeModelId }, modelRequestOptions);
    const chatOptions = {
    ...
    if (Number.isFinite(timeoutMs) && timeoutMs > 0) {
      singleRequestOptions.timeout = timeoutMs;
    }
    if (options?.abortSignal) {
      singleRequestOptions.signal = options.abortSignal;
    }
    if (requestHeaders) {
      singleRequestOptions.customHeaders = requestHeaders;
    }
    const run = async () => {
      try {
        const result = await chat.sendMessage(
          normalizedPayload.lastMessageParts,
        ...
        if (isAbort) {
          const abortErr = new Error('Gemini request aborted by caller.');
  ...
  try {
    ...
    if (isModelNotFoundError(error)) {
      ...
      if (retriedModel && retriedModel !== modelToUse) {
        if (!s(credentials.apiKey) && !disableCache) cachedDefaultModelId = retriedModel;
...
function healthCheck(options = {}) {
  ...
  return {
    ...
    cachedDefaultModelId: cachedDefaultModelId || null,
    cache: {
      modelListCached: Boolean(cachedModelRows),
      modelListAgeMs: cachedModelRowsAt ? Math.max(0, now() - cachedModelRowsAt) : null
    },
