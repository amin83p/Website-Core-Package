...
const assignmentModel = require('../models/pte/pteApplicantPackageAssignmentModel');
...
async function generateMongoAssignmentId(collection, requestedId = null, isoDateTime = null) {
  ...
  for (let i = 0; i < 250; i += 1) {
    ...
    const candidate = `PTEPKG${dateToken}${suffix}`;
...
async function listMongoRows(options = {}) {
  const collection = getMongoCollection('pteApplicantPackageAssignments');
...
const pteApplicantPackageAssignmentRepository = {
  async list(options = {}) {
  ...
  async getById(id, options = {}) {
    return runByRepositoryBackend(options, {
      ...
      mongo: async () => normalizeMongoDocument(await getMongoCollection('pteApplicantPackageAssignments').findOne(resolveMongoIdFilter(id)))
    }, 'pte.assignments.getById');
  },
  ...
  async create(data, options = {}) {
    ...
    }, 'pte.assignments.create');
    await actionStateChangeTrackerService.trackCreate({
      source: 'pte',
      entityType: 'pteApplicantPackageAssignments',
  ...
  async update(id, data, options = {}) {
    ...
    const updated = await runByRepositoryBackend(options, {
      ...
      mongo: async () => {
        ...
        if (!existing) throw new Error('PTE applicant package assignment not found.');
  ...
  async remove(id, options = {}) {
    return runByRepositoryBackend(options, {
      ...
      mongo: async () => getMongoCollection('pteApplicantPackageAssignments').deleteOne(resolveMongoIdFilter(id))
    }, 'pte.assignments.remove');
  }
};

assertQueryableCrudRepository('pteApplicantPackageAssignmentRepository', pteApplicantPackageAssignmentRepository);

module.exports = pteApplicantPackageAssignmentRepository;
