...
const SCRIPT_ID = 'packages/benchpath/scripts/maintenance/enable-benchpath-package.js';
...
function loadBenchpathManifest() {
...
function printHumanReport(report = {}) {
  ...
  console.log(`[${mode}] BenchPath package registry activation`);
...
async function runEnableBenchpathPackage(argv = [], runtimeOptions = {}) {
  const options = parseArgs(Array.isArray(argv) ? argv : []);
  const emit = runtimeOptions.emit !== false;

  if (options.help) {
    if (emit) {
      console.log('Usage: node packages/benchpath/scripts/maintenance/enable-benchpath-package.js [--apply] [--disable|--remove] [--json]');
  ...
  const manifest = loadBenchpathManifest();
...
async function main() {
  await runEnableBenchpathPackage(process.argv.slice(2), { emit: true });
}
...
module.exports = {
  parseArgs,
  loadBenchpathManifest,
  countDeclarations,
  buildRegistryPayload,
  initializeRegistryBackend,
  runEnableBenchpathPackage,
  resolveCoreRoot
};
