const SECTION_ID = '445578';
const SECTION_NAME = 'SCHOOL_PRIOR_SUBJECT_CREDITS';
const SECTION_HOME_URL = '/school/programs/prior-subject-credits';
const PARENT_SECTION_ID = '139382';
const PARENT_SECTION_NAME = 'SCHOOL_ACADEMIA';
const SYMBOL_ID = 'SYM_SYSTEM_084';
const ACCESS_PROFILES = ['SCHOOL_STAFF'];

const orgOperations = ['OP1001', 'OP1002', 'OP1003', 'OP1005'].map((operationId) => ({
  operationId,
  scopeId: 'SCP_ORG',
...
const sectionRow = {
  ...
  name: SECTION_NAME,
  category: 'SCHOOL',
  description: 'Manage student prior subject credits that can satisfy prerequisite and progression checks.',
  active: true,
  trackState: true,
...
function ensureParentSubsection() {
  ...
  if (!parent) {
    print(`Parent section ${PARENT_SECTION_NAME} was not found; prior subject credits section was not attached.`);
    return;
  }
  ...
  subsections.forEach((row) => {
    next.push(row);
    if (!inserted && String(row && row.id) === '445564') {
  ...
  const matched = db.accesses.updateOne(
    { name: profileName, 'sections.sectionId': SECTION_ID },
    {
      $set: {
        ...
        'sections.$.operations': orgOperations,
        updatedAt: new Date(),
        updatedBy: 'system'
      }
    }
...
print('School prior subject credits section seed complete.');
