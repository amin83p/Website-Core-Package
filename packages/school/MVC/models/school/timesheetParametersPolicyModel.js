...
const dataPath = path.join(resolveCoreRoot(), 'data/school/timesheetParametersPolicy.json');

/** Must match jsonToMongoMigrationService transform for school.timesheetParametersPolicy */
const MONGO_COLLECTION = 'schoolTimesheetParametersPolicy';
const MONGO_DOC_ID = 'timesheet-parameters-policy';

const { DEFAULT_POLICY } = timesheetParametersPolicyService;
...
function effectivePolicyFromDoc(doc, activeOrgId) {
  ...
  if (!row || typeof row !== 'object') {
    return timesheetParametersPolicyService.resolvePolicy(DEFAULT_POLICY);
  }
  return timesheetParametersPolicyService.resolvePolicy(
    timesheetParametersPolicyService.normalizePolicyFromStored(pickStoredPolicyFields(row))
  );
}
...
async function getPolicyForOrg(activeOrgId) {
  ...
  }, 'school.timesheetParametersPolicy.getPolicyForOrg');
}

async function savePolicyForOrg(activeOrgId, patch, auditUserId) {
  const normalized = timesheetParametersPolicyService.validatePolicyInput(patch);
  ...
  }, 'school.timesheetParametersPolicy.savePolicyForOrg');
  return normalized;
}
...
async function removePolicyForOrg(activeOrgId) {
  ...
  }, 'school.timesheetParametersPolicy.removePolicyForOrg');
}
...
async function readPolicyDocument() {
  ...
  }, 'school.timesheetParametersPolicy.readDocument');
}
...
async function getStoredPolicyRowForOrg(activeOrgId) {
  ...
  const resolved = timesheetParametersPolicyService.resolvePolicy(
    timesheetParametersPolicyService.normalizePolicyFromStored(pickStoredPolicyFields(stored))
  );
  return {
    id: key,
    orgId: key,
    emptyEnrollmentSessions: resolved.emptyEnrollmentSessions,
    statutoryHolidayPay: resolved.statutoryHolidayPay,
    status: 'stored',
