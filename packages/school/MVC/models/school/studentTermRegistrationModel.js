...
const dataPath = path.join(resolveCoreRoot(), 'data/school/studentTermRegistrations.json');
...
function sanitizeRegistrationInput(input, { isUpdate = false } = {}) {
  if (!isPlainObject(input)) throw new Error('Invalid student term registration payload.');
  ...
  const termId = cleanId(input.termId, { max: 64, allowEmpty: false });
  if (!orgId || !studentId || !personId || !programId || !termId) {
    throw new Error('orgId, studentId, personId, programId, and termId are required.');
  }

  const status = cleanString(input.status, { max: 20, allowEmpty: true }).toLowerCase() || 'registered';
  if (!REGISTRATION_STATUSES.has(status)) throw new Error('Invalid student term registration status.');
  const classEnrollmentSummary = isPlainObject(input.classEnrollmentSummary)
    ? input.classEnrollmentSummary
    : (isPlainObject(input.rosterSummary) ? input.rosterSummary : {});
  ...
  const out = {
    ...
    note: cleanString(input.note, { max: 4000, allowEmpty: true }),
...
function generateId(existingIds) {
  for (let i = 0; i < 50; i++) {
    const candidate = `STR-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
    if (!existingIds.has(candidate)) return candidate;
  }
  return `STR-${Date.now()}-${Math.random().toString(36).slice(2, 7).toUpperCase()}`;
}
...
async function getAllRegistrations() {
  try {
    ...
    throw new Error('Failed to retrieve student term registrations.');
  }
}
...
async function findByStudentProgramAndTerm(studentId, programId, termId) {
  const all = await getAllRegistrations();
  return all.filter((row) =>
    idsEqual(row?.studentId || '', studentId || '') &&
    idsEqual(row?.programId || '', programId || '') &&
    idsEqual(row?.termId || '', termId || '')
  );
}
...
async function updateRegistration(id, data, options = {}) {
  ...
  return queueWrite(async () => {
    ...
    if (index === -1) throw new Error('Student term registration not found.');
...
async function clearRegistrationsByOrg(orgId) {
  return queueWrite(async () => {
    ...
    if (!targetOrgId) throw new Error('orgId is required to clear student term registrations.');
...
async function deleteRegistration(id, options = {}) {
  ...
  return queueWrite(async () => {
    ...
    if (filtered.length === all.length) throw new Error('Student term registration not found.');
