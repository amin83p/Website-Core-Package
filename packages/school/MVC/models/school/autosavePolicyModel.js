...
const autosavePolicyService = require('../../services/school/autosavePolicyService');

const dataPath = path.join(resolveCoreRoot(), 'data/school/autosavePolicy.json');

/** Must match jsonToMongoMigrationService transform for school.autosavePolicy */
const MONGO_COLLECTION = 'schoolAutosavePolicy';
const MONGO_DOC_ID = 'autosave-policy';

const { DEFAULT_POLICY } = autosavePolicyService;
...
function effectivePolicyFromDoc(doc, activeOrgId) {
  ...
  if (!row || typeof row !== 'object') {
    return autosavePolicyService.resolvePolicy(DEFAULT_POLICY);
  }
  return autosavePolicyService.resolvePolicy(
    autosavePolicyService.normalizePolicyFromStored(pickStoredPolicyFields(row))
  );
}
...
async function getPolicyForOrg(activeOrgId) {
  ...
  }, 'school.autosavePolicy.getPolicyForOrg');
}

async function savePolicyForOrg(activeOrgId, patch, auditUserId) {
  const normalized = autosavePolicyService.validatePolicyInput(patch);
  ...
  }, 'school.autosavePolicy.savePolicyForOrg');
  return normalized;
}
...
async function removePolicyForOrg(activeOrgId) {
  ...
  }, 'school.autosavePolicy.removePolicyForOrg');
}
...
async function readPolicyDocument() {
  ...
  }, 'school.autosavePolicy.readDocument');
}
...
async function getStoredPolicyRowForOrg(activeOrgId) {
  ...
  const resolved = autosavePolicyService.resolvePolicy(
    autosavePolicyService.normalizePolicyFromStored(pickStoredPolicyFields(stored))
  );
  return {
    id: key,
    orgId: key,
    defaultMinutes: resolved.defaultMinutes,
    sectionCount: Object.keys(resolved.sections || {}).length,
    status: 'stored',
