...
const dataPath = path.join(resolveCoreRoot(), 'data/school/attendanceMarkAppearancePolicy.json');

/** Must match jsonToMongoMigrationService transform for school.attendanceMarkAppearancePolicy */
const MONGO_COLLECTION = 'schoolAttendanceMarkAppearancePolicy';
const MONGO_DOC_ID = 'attendance-mark-appearance-policy';

const { DEFAULT_POLICY } = attendanceMarkAppearanceService;
...
function effectivePolicyFromDoc(doc, activeOrgId) {
  ...
  if (!row || typeof row !== 'object') {
    return attendanceMarkAppearanceService.resolvePolicy(DEFAULT_POLICY);
  }
  return attendanceMarkAppearanceService.resolvePolicy(
    attendanceMarkAppearanceService.normalizePolicyFromStored(pickStoredPolicyFields(row))
  );
}
...
async function getPolicyForOrg(activeOrgId) {
  ...
  }, 'school.attendanceMarkAppearancePolicy.getPolicyForOrg');
}

async function savePolicyForOrg(activeOrgId, patch, auditUserId) {
  const normalized = attendanceMarkAppearanceService.normalizePolicyFromForm(patch);
  ...
  }, 'school.attendanceMarkAppearancePolicy.savePolicyForOrg');
  return normalized;
}
...
async function removePolicyForOrg(activeOrgId) {
  ...
  }, 'school.attendanceMarkAppearancePolicy.removePolicyForOrg');
}
...
async function readPolicyDocument() {
  ...
  }, 'school.attendanceMarkAppearancePolicy.readDocument');
}
...
async function getStoredPolicyRowForOrg(activeOrgId) {
  ...
  const resolved = attendanceMarkAppearanceService.resolvePolicy(
    attendanceMarkAppearanceService.normalizePolicyFromStored(pickStoredPolicyFields(stored))
  );
  const marks = Array.isArray(resolved?.marks) ? resolved.marks : [];
  return {
    id: key,
    orgId: key,
    marks,
