...
const dataPath = path.join(resolveCoreRoot(), 'data/school/studentAttendanceReportPolicy.json');

/** Must match jsonToMongoMigrationService transform for school.studentAttendanceReportPolicy */
const MONGO_COLLECTION = 'schoolStudentAttendanceReportPolicy';
const MONGO_DOC_ID = 'student-attendance-report-policy';

const { DEFAULT_POLICY } = studentAttendanceReportPolicyService;
...
function effectivePolicyFromDoc(doc, activeOrgId) {
  ...
  if (!row || typeof row !== 'object') {
    return studentAttendanceReportPolicyService.resolvePolicy(DEFAULT_POLICY);
  }
  return studentAttendanceReportPolicyService.resolvePolicy(
    studentAttendanceReportPolicyService.normalizePolicyFromStored(pickStoredPolicyFields(row))
  );
}
...
async function getPolicyForOrg(activeOrgId) {
  ...
  }, 'school.studentAttendanceReportPolicy.getPolicyForOrg');
}

async function savePolicyForOrg(activeOrgId, patch, auditUserId) {
  const normalized = studentAttendanceReportPolicyService.normalizePolicyFromForm(patch);
  ...
  }, 'school.studentAttendanceReportPolicy.savePolicyForOrg');
  return normalized;
}
...
async function removePolicyForOrg(activeOrgId) {
  ...
  }, 'school.studentAttendanceReportPolicy.removePolicyForOrg');
}
...
async function readPolicyDocument() {
  ...
  }, 'school.studentAttendanceReportPolicy.readDocument');
}
...
async function getStoredPolicyRowForOrg(activeOrgId) {
  ...
  const resolved = studentAttendanceReportPolicyService.resolvePolicy(
    studentAttendanceReportPolicyService.normalizePolicyFromStored(pickStoredPolicyFields(stored))
  );
  return {
    id: key,
    orgId: key,
    ...
    templateExportFormats: resolved.templateExportFormats,
    status: 'stored',
