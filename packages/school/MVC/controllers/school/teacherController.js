...
exports.listTeachers = async (req, res) => {
  try {
    ...
    // Enrichment search must scan the full teacher set, then paginate in memory.
...
exports.saveTeacher = async (req, res) => {
  ...
  try {
    ...
    if (id) {
      ...
      const person = await schoolPersonAccessService.getPersonById({ reqUser: req.user, personId: savedTeacher.personId });
...
async function assertCanChangeTeacherSystemId(req) {
  const allowed = await adminAuthorityService.isAdminForRequestAsync(req.user, SECTIONS.SCHOOL_TEACHERS, OPERATIONS.UPDATE, { section: { id: SECTIONS.SCHOOL_TEACHERS } });
...
exports.deleteTeacher = async (req, res) => {
  ...
  try {
    ...
    const person = teacher?.personId
      ? await schoolPersonAccessService.getPersonById({ reqUser: req.user, personId: teacher.personId })
      : null;
    ...
    await dataService.purgeData('teachers', teacher.id, req.user, {
      transactionContext: txContext,
      skipDeletionGuard: true
    });
    if (txContext) {
