...
exports.listPeoplePanel = async (req, res) => {
  try {
    ...
    const result = await reportHubService.getPeoplePanelRows(type, req.query, req);
    res.json({ status: 'success', ...result });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    res.status(statusCode).json({
      status: 'error',
      message: error.message || 'Unable to load Report Hub list.'
    });
  }
};
...
exports.getTaskCount = async (req, res) => {
  try {
    const result = await reportHubService.getTaskSummary(req);
    res.json({ status: 'success', ...result });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    res.status(statusCode).json({
...
exports.getWorkspaceSection = async (req, res) => {
  try {
    ...
    const result = await reportHubService.getWorkspaceSection(sectionKey, req.query, req);
    res.json({ status: 'success', ...result });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    res.status(statusCode).json({
      status: 'error',
      message: error.message || 'Unable to load this Report Hub section.'
    });
  }
};
...
exports.lockWorkspaceSessions = async (req, res) => {
  try {
    const result = await reportHubService.lockWorkspaceSessions(req.body || {}, req);
    res.json({
      status: 'success',
...
exports.updateWorkspaceSession = async (req, res) => {
  try {
    const result = await reportHubService.updateWorkspaceSession(req.body || {}, req);
    res.json({
      status: 'success',
