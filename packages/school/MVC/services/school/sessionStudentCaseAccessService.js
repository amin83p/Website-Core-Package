...
async function resolveCaseCapabilities(req, { classData = null, session = null } = {}) {
  const [
    canCreateOp,
    canReadOp,
    canReadAllOp,
  ...
  const canResolve = canResolveOp && sessionMutationOk;
...
async function assertCanRead(req, classData, session) {
  const capabilities = await resolveCaseCapabilities(req, { classData, session });
