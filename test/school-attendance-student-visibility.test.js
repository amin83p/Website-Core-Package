...
test('attendance matrix save flow uses local cell patching and request-budget guards', () => {
  ...
  assert.match(view, /setAttendanceBudgetState\(window\.ATTENDANCE_ACTION_LIMITS/);
  assert.doesNotMatch(view, /ATTENDANCE_MATRIX_UI_REQUEST_LIMIT_OVERRIDE/);
  assert.doesNotMatch(view, /ATTENDANCE_MATRIX_MUTATION_BUDGET_SOURCES/);
