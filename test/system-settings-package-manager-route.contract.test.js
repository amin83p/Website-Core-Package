...
test('package manager GET route reaches controller with system settings update scope', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use('/systemSettings', router);

    await withServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/systemSettings/packages`, {
        method: 'GET',
        headers: {
      ...
      assert.equal(body.handler, 'showPackageManagerPage');
...
test('package manager install POST requires actionState token and admin approval', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    ...
    await withServer(app, async (baseUrl) => {
      const missingToken = await fetch(`${baseUrl}/systemSettings/packages/install`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      const missingAdmin = await fetch(`${baseUrl}/systemSettings/packages/install`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      const allowed = await fetch(`${baseUrl}/systemSettings/packages/install`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({
          installMethod: 'path',
          manifestPath: 'packages/pte/package.manifest.json',
          actionStateId: 'STATE_2'
        }).toString()
      });
      assert.equal(allowed.status, 200);
      const body = await allowed.json();
      assert.equal(body.handler, 'installPackageFromManager');
...
test('package manager remove POST keeps protected middleware contract', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    ...
    await withServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/systemSettings/packages/pte/remove`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_REMOVE' }).toString()
      });
      assert.equal(response.status, 200);
      const body = await response.json();
...
test('package manager uninstall-preview POST keeps protected middleware contract', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use(express.urlencoded({ extended: false }));
    app.use('/systemSettings', router);
    ...
    await withServer(app, async (baseUrl) => {
      const missingToken = await fetch(`${baseUrl}/systemSettings/packages/pte/uninstall-preview`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      const allowed = await fetch(`${baseUrl}/systemSettings/packages/pte/uninstall-preview`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_PREVIEW' }).toString()
      });
      assert.equal(allowed.status, 200);
      const body = await allowed.json();
      assert.equal(body.handler, 'uninstallPreviewPackageFromManager');
      assert.equal(body.actionState.requireToken, true);
    });
  });
});

test('package manager transactions GET uses package manager access scope', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use('/systemSettings', router);

    await withServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/systemSettings/packages/pte/transactions?limit=10`, {
        method: 'GET',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes'
