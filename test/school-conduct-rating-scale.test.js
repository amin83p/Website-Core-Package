...
test('session manager places optional conduct after attendance and Step 1 only in reports', () => {
  ...
  assert.match(source, /conduct-bulk-modal-dialog/);
  assert.match(source, /display: inline-flex/);
  assert.match(source, /flex-wrap: nowrap/);
  assert.match(source, /#conductTable tbody tr/);
  assert.match(source, /border-left: 1px solid #e2e8f0/);
  assert.match(source, /white-space: nowrap/);
  assert.match(source, /conduct-column-actions/);
