...
test('school package pass15 owns sessionController implementation', () => {
  ...
  assert.match(source, /require\('\.\.\/\.\.\/services\/school\/sessionExplorerService'\)/);
  assert.match(source, /sessionExplorerService\.buildSessionExplorerViewer/);
...
test('school package pass16 owns payRateController implementation', () => {
  ...
  assert.match(source, /require\('\.\.\/\.\.\/services\/school\/schoolDataService'\)/);
  assert.match(source, /requireCoreModule\('MVC\/utils\/idAdapter'\)/);
...
test('school package pass21 owns scheduleController implementation', () => {
  const source = read('packages/school/MVC/controllers/school/scheduleController.js');
  assert.equal(source.includes("requireCoreModule('MVC/controllers/school/scheduleController.js')"), false);
  assert.match(source, /require\('\.\.\/\.\.\/services\/school\/schoolDataService'\)/);
  assert.match(source, /requireCoreModule\('MVC\/utils\/idAdapter'\)/);
  assert.match(source, /module\.exports\s*=\s*\{/);
});
...
test('school package pass23 owns termRegistrationController implementation', () => {
  const source = read('packages/school/MVC/controllers/school/termRegistrationController.js');
  assert.equal(source.includes("requireCoreModule('MVC/controllers/school/termRegistrationController.js')"), false);
  assert.match(source, /requireCoreModule\('MVC\/utils\/paginationHelper'\)/);
  assert.match(source, /requireCoreModule\('MVC\/services\/transactionContextService'\)/);
  assert.match(source, /exports\.applyRegistration/);
});
test('school package pass24 owns examController implementation', () => {
  const source = read('packages/school/MVC/controllers/school/examController.js');
  assert.equal(source.includes("requireCoreModule('MVC/controllers/school/examController.js')"), false);
  assert.match(source, /requireCoreModule\('MVC\/middleware\/upload'\)/);
  assert.match(source, /requireCoreModule\('MVC\/services\/uploadFolderSettingsService'\)/);
  assert.match(source, /module\.exports\s*=\s*\{/);
});
test('school package pass25 owns classController implementation', () => {
  const source = read('packages/school/MVC/controllers/school/classController.js');
  assert.equal(source.includes("requireCoreModule('MVC/controllers/school/classController.js')"), false);
  assert.match(source, /requireCoreModule\('MVC\/services\/dataService'\)/);
  assert.match(source, /requireCoreModule\('MVC\/services\/security\/index'\)/);
  assert.match(source, /module\.exports\s*=\s*\{/);
});
