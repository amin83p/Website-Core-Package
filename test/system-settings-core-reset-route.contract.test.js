...
test('core reset GET route uses dedicated system core reset update scope', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use('/systemSettings', router);

    await withServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/systemSettings/core-reset`, {
        method: 'GET',
        headers: {
...
test('core reset preflight POST requires action state token', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    ...
    await withServer(app, async (baseUrl) => {
      const missingToken = await fetch(`${baseUrl}/systemSettings/core-reset/preflight`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      const allowed = await fetch(`${baseUrl}/systemSettings/core-reset/preflight`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_RESET_PRE_1' }).toString()
      });
      assert.equal(allowed.status, 200);
      const body = await allowed.json();
      assert.equal(body.handler, 'preflightCoreReset');
      assert.equal(body.actionState.requireToken, true);
    });
  });
});

test('core reset apply POST requires token and admin approval', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use(express.urlencoded({ extended: false }));
    app.use('/systemSettings', router);
    ...
    await withServer(app, async (baseUrl) => {
      const missingToken = await fetch(`${baseUrl}/systemSettings/core-reset/apply`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      const missingAdmin = await fetch(`${baseUrl}/systemSettings/core-reset/apply`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_RESET_APPLY_1', confirmToken: 'RESET CORE' }).toString()
      });
      assert.equal(missingAdmin.status, 403);
      assert.equal((await missingAdmin.json()).status, 'admin_required');

      const allowed = await fetch(`${baseUrl}/systemSettings/core-reset/apply`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_RESET_APPLY_2', confirmToken: 'RESET CORE' }).toString()
      });
      assert.equal(allowed.status, 200);
      const body = await allowed.json();
      assert.equal(body.handler, 'applyCoreReset');
      assert.equal(body.actionState.requireToken, true);
      assert.equal(body.actionState.actionStateId, 'STATE_RESET_APPLY_2');
    });
  });
});
