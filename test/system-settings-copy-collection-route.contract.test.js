...
test('copy-collection GET route reaches controller and uses system settings update scope', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use('/systemSettings', router);

    await withServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/systemSettings/data-migration/copy-collection`, {
        method: 'GET',
        headers: {
      ...
      assert.equal(body.handler, 'showDataMigrationCopyCollectionPage');
...
test('copy-collection overwrite POST requires actionState token and admin approval', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    ...
    await withServer(app, async (baseUrl) => {
      const missingToken = await fetch(`${baseUrl}/systemSettings/data-migration/copy-collection/overwrite`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ collection: 'users', destinationUri: 'mongodb://example' }).toString()
      });
      assert.equal(missingToken.status, 403);
      assert.equal((await missingToken.json()).status, 'token_required');

      const missingAdmin = await fetch(`${baseUrl}/systemSettings/data-migration/copy-collection/overwrite`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({
          collection: 'users',
          destinationUri: 'mongodb://example',
      ...
      const allowed = await fetch(`${baseUrl}/systemSettings/data-migration/copy-collection/overwrite`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      assert.equal(body.handler, 'overwriteDataMigrationCollection');
