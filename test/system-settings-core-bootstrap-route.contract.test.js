...
test('core bootstrap GET route uses system settings update scope', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use('/systemSettings', router);

    await withServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/systemSettings/bootstrap/core`, {
        method: 'GET',
        headers: {
      ...
      assert.equal(body.handler, 'showCoreBootstrapPage');
...
test('core bootstrap preflight POST requires action state token', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    ...
    await withServer(app, async (baseUrl) => {
      const missingToken = await fetch(`${baseUrl}/systemSettings/bootstrap/core/preflight`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      const allowed = await fetch(`${baseUrl}/systemSettings/bootstrap/core/preflight`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_BOOT_PRE_1' }).toString()
      });
      assert.equal(allowed.status, 200);
      const body = await allowed.json();
      assert.equal(body.handler, 'preflightCoreBootstrapBaseline');
      assert.equal(body.actionState.requireToken, true);
    });
  });
});

test('core bootstrap apply POST requires token and admin approval', async () => {
  await withStubbedSystemSettingsRoutes(async (router) => {
    const app = express();
    app.use(express.urlencoded({ extended: false }));
    app.use('/systemSettings', router);
    ...
    await withServer(app, async (baseUrl) => {
      const missingToken = await fetch(`${baseUrl}/systemSettings/bootstrap/core/apply`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
      ...
      const missingAdmin = await fetch(`${baseUrl}/systemSettings/bootstrap/core/apply`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_BOOT_APPLY_1' }).toString()
      });
      assert.equal(missingAdmin.status, 403);
      assert.equal((await missingAdmin.json()).status, 'admin_required');

      const allowed = await fetch(`${baseUrl}/systemSettings/bootstrap/core/apply`, {
        method: 'POST',
        headers: {
          authorization: 'Bearer allowed',
          'x-allow-access': 'yes',
        ...
        body: new URLSearchParams({ actionStateId: 'STATE_BOOT_APPLY_2' }).toString()
      });
      assert.equal(allowed.status, 200);
      const body = await allowed.json();
      assert.equal(body.handler, 'applyCoreBootstrapBaseline');
      assert.equal(body.actionState.requireToken, true);
      assert.equal(body.actionState.actionStateId, 'STATE_BOOT_APPLY_2');
    });
  });
});
