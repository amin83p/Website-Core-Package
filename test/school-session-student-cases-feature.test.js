...
test('session student case access services centralize on SCHOOL_SESSION_STUDENT_CASES only', () => {
  const accessBuilder = read('packages/school/MVC/services/school/studentCaseAccessService.js');
  const policy = read('packages/school/MVC/services/school/studentCaseOperationPolicyService.js');
  const accessService = read('packages/school/MVC/services/school/sessionStudentCaseAccessService.js');
  assert.match(accessBuilder, /SECTIONS\.SCHOOL_SESSION_STUDENT_CASES/);
  assert.match(accessBuilder, /buildStudentCaseSectionAccessContext/);
  assert.match(policy, /SECTIONS\.SCHOOL_SESSION_STUDENT_CASES/);
  assert.match(accessService, /buildStudentCaseAccess/);
  assert.doesNotMatch(accessBuilder, /SCHOOL_SESSIONS/);
  assert.doesNotMatch(policy, /SCHOOL_SESSIONS/);
});
...
test('session manager renders student cases tab, modal wizard, and avoids attendance duplicate fields', () => {
  ...
  assert.match(modalBody, /id="studentCaseStepStudents"/);
  assert.match(modalBody, /id="studentCaseStepDetails"/);
  assert.match(modalFooter, /id="btnStudentCaseNext"/);
  assert.match(modalFooter, /id="btnSaveStudentCaseSessionWide"/);
  assert.match(clientSource, /function advanceWizard\(/);
  assert.doesNotMatch(src, /btn-open-student-case/);
  assert.match(src, /canManageClassConduct/);
  assert.match(modalFooter, /id="btnResolveStudentCase"/);
  assert.match(clientSource, /saveCase\(\{ resolve: true \}\)/);
  ...
  assert.match(clientSource, /studentCaseDetailPresets.*addEventListener\('change'/s);
  assert.match(clientSource, /getElementById\('studentCaseDetails'\)/);
