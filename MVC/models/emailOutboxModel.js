...
const DATA_PATH = path.join(__dirname, '../../data/emailOutbox.json');
...
function generateId(existingRows = []) {
  ...
  for (let i = 0; i < 300; i += 1) {
    ...
    const candidate = `EMOBX${dateToken}${suffix}`;
    if (!existing.has(candidate)) return candidate;
  }
  return `EMOBX${Date.now()}`;
}

function normalizeEmailOutboxRecord(record = {}, existing = null, { strict = false } = {}) {
  ...
  const normalized = {
    ...
    to: cleanText(input.to || base.to, { max: 320, allowEmpty: true }),
    subject: cleanText(input.subject || base.subject, { max: 500, allowEmpty: true }),
    html: cleanText(input.html || base.html, { max: 200000, allowEmpty: true }),
    text: cleanText(input.text || base.text, { max: 100000, allowEmpty: true }),
    from: cleanText(input.from || base.from, { max: 320, allowEmpty: true }),
    replyTo: cleanText(input.replyTo || base.replyTo, { max: 320, allowEmpty: true }),
    providerProfileId: cleanText(input.providerProfileId || base.providerProfileId, { max: 120, allowEmpty: true }),
    templateId: cleanText(input.templateId || base.templateId, { max: 120, allowEmpty: true }),
    injectedValues: input.injectedValues !== undefined ? input.injectedValues : (base.injectedValues || {}),
  ...
  if (strict) {
    if (!normalized.to) throw new Error('Email recipient is required.');
    if (!normalized.subject) throw new Error('Email subject is required.');
    if (!normalized.text && !normalized.html) throw new Error('Email body is required.');
  }
...
async function readAll() {
  try {
    ...
    throw new Error('Failed to retrieve email outbox entries.');
  }
}
...
async function add(record = {}) {
  return queueWrite(async () => {
    const rows = await readAll();
    const normalized = normalizeEmailOutboxRecord(record, null, { strict: true });
...
async function update(id, patch = {}) {
  return queueWrite(async () => {
    ...
    if (index < 0) throw new Error('Email outbox entry not found.');
    const normalized = normalizeEmailOutboxRecord(patch, rows[index]);
