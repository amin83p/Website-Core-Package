...
const DATA_PATH = path.join(__dirname, '../../data/activityQuotaPackageAssignments.json');
const STATUS_VALUES = new Set(['active', 'paused', 'removed']);
...
function cleanIdList(values = []) {
  ...
  const seen = new Set();
  rows.forEach((value) => {
    const id = cleanId(value, { max: 180, allowEmpty: true }) || '';
...
function cleanRoleTokens(values = []) {
  const rows = Array.isArray(values) ? values : [values];
  const out = [];
  const seen = new Set();
  rows.forEach((value) => {
    ...
    if (!/^[a-z0-9_.:-]+$/.test(token)) {
      throw new Error('eligibleRoles contains invalid role token.');
    }
...
function sanitizeAssignment(raw = {}, options = {}) {
  ...
  const targetUserId = cleanId(input.targetUserId || existing?.targetUserId, { max: 120, allowEmpty: false });
  ...
  return {
    ...
    orgId,
    targetUserId,
    targetUserName: cleanString(input.targetUserName || existing?.targetUserName, { max: 220, allowEmpty: true }) || '',
    packageId,
    ...
    eligibleRoles: cleanRoleTokens(input.eligibleRoles !== undefined ? input.eligibleRoles : existing?.eligibleRoles || []),
    ...
    status,
    notes: cleanString(input.notes || existing?.notes, { max: 3000, allowEmpty: true }) || '',
    creator,
    audit
  };
}
...
function generateId(rows = []) {
  ...
  for (let i = 0; i < 300; i += 1) {
    const candidate = `AQPA${Date.now()}${Math.floor(Math.random() * 1000)}`;
...
async function getAllAssignments() {
  try {
    ...
    throw new Error('Failed to retrieve activity quota package assignments.');
  }
}
...
async function addAssignment(payload) {
  return queueWrite(async () => {
    ...
    if (rows.some((row) => idsEqual(row?.id, sanitized.id))) {
      throw new Error(`Activity quota package assignment id '${sanitized.id}' already exists.`);
    }
    rows.push(sanitized);
...
async function updateAssignment(id, payload) {
  return queueWrite(async () => {
    ...
    if (index < 0) throw new Error('Activity quota package assignment not found.');
