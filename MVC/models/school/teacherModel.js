// MVC/models/school/teacherModel.js
const fs = require('fs').promises;
const fsSync = require('fs');
const path = require('path');
const { queueWrite } = require('../fileQueue');

const dataPath = path.join(__dirname, '../../../data/school/teachers.json');

if (!fsSync.existsSync(dataPath)) {
  fsSync.writeFileSync(dataPath, '[]');
}

const TEACHER_STATUSES = new Set(['Active', 'On Leave', 'Inactive', 'Archived']);
const EMPLOYMENT_TYPES = new Set(['Full-Time', 'Part-Time', 'Contract', 'Sessional', 'Substitute']);
const INSTRUCTIONAL_MODES = new Set(['In-person', 'Online', 'Hybrid']);
...
function sanitizeTeacherInput(input, { isUpdate = false } = {}) {
  ...
  if (!TEACHER_STATUSES.has(status)) throw new Error('Invalid teacher status.');
  ...
  const instructionalMode = cleanString(input.instructionalMode, { max: 20, allowEmpty: true }) || 'In-person';
  if (!INSTRUCTIONAL_MODES.has(instructionalMode)) throw new Error('Invalid instructional mode.');

  const out = {
    orgId: String(orgId),
    personId: String(personId),
    teacherAccountId: cleanId(input.teacherAccountId, { max: 64, allowEmpty: true }),
...
async function addTeacher(data, options = {}) {
  void options;
  return queueWrite(async () => {
    const all = await getAllTeachers();
    const sanitized = sanitizeTeacherInput(data, { isUpdate: false });

    if (all.some((t) => String(t.orgId) === String(sanitized.orgId) && String(t.personId) === String(sanitized.personId))) {
      throw new Error('This person is already registered as a teacher.');
    }

    const existingIds = new Set(all.map((t) => String(t.id)));
    const finalId = sanitized.id ? String(sanitized.id) : generateTeacherId(existingIds);
    if (existingIds.has(finalId)) throw new Error('Teacher id already exists.');
...
async function updateTeacher(id, data, options = {}) {
  void options;
  return queueWrite(async () => {
    const all = await getAllTeachers();
    const index = all.findIndex((t) => String(t.id) === String(id));
    if (index === -1) throw new Error('Teacher not found');

    const existing = all[index];
    const sanitized = sanitizeTeacherInput(
      {
        ...data,
...
async function deleteTeacher(id, options = {}) {
  void options;
  return queueWrite(async () => {
    const all = await getAllTeachers();
    const index = all.findIndex((t) => String(t.id) === String(id));
...
async function purgeTeacher(id, options = {}) {
  void options;
  return queueWrite(async () => {
    const all = await getAllTeachers();
    const index = all.findIndex((t) => String(t.id) === String(id));
...
module.exports = {
  ...
  deleteTeacher,
  purgeTeacher,
  TEACHER_STATUSES: Object.freeze([...TEACHER_STATUSES]),
