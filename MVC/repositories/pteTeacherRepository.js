...
const pteTeacherModel = require('../models/pte/pteTeacherModel');
...
async function generateMongoTeacherId(collection, requestedId = null, isoDateTime = null) {
...
async function listMongoRows(options = {}) {
  const collection = getMongoCollection('pteTeachers');
...
const pteTeacherRepository = {
  async list(options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => {
        const rows = await pteTeacherModel.getAllTeachers();
  ...
  async getById(id, options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => pteTeacherModel.getTeacherById(id),
      mongo: async () => normalizeMongoDocument(await getMongoCollection('pteTeachers').findOne(resolveMongoIdFilter(id)))
    }, 'pte.teachers.getById');
  },

  async create(data, options = {}) {
    const created = await runByRepositoryBackend(options, {
      json: async () => pteTeacherModel.addTeacher(data),
      mongo: async () => {
        const collection = getMongoCollection('pteTeachers');
        const payload = { ...(data || {}) };
        payload.id = await generateMongoTeacherId(collection, payload.id, payload?.audit?.createDateTime || new Date().toISOString());
  ...
  async update(id, data, options = {}) {
    ...
    const updated = await runByRepositoryBackend(options, {
      json: async () => pteTeacherModel.updateTeacher(id, data),
      mongo: async () => {
        const collection = getMongoCollection('pteTeachers');
        const existing = await collection.findOne(resolveMongoIdFilter(id));
        if (!existing) throw new Error('PTE teacher not found.');
  ...
  async remove(id, options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => pteTeacherModel.deleteTeacher(id),
      mongo: async () => getMongoCollection('pteTeachers').deleteOne(resolveMongoIdFilter(id))
    }, 'pte.teachers.remove');
  }
};

assertQueryableCrudRepository('pteTeacherRepository', pteTeacherRepository);

module.exports = pteTeacherRepository;
