...
const model = require('../models/smsOutboxModel');
...
const DEFAULT_SEARCH_FIELDS = Object.freeze(['id', 'orgId', 'eventKey', 'to', 'body', 'status', 'dedupeKey']);
...
const smsOutboxRepository = {
  ...
  async create(data, options = {}) {
    return runByRepositoryBackend(options, {
      ...
      mongo: async () => {
        const collection = getMongoCollection(COLLECTION_NAME);
        const normalized = model.normalizeSmsOutboxRecord(data, null, { strict: true });
  ...
  async createMany(rows = [], options = {}) {
    ...
    return runByRepositoryBackend(options, {
      ...
      mongo: async () => {
        ...
        const payloads = [];
        for (const row of rows) {
          const normalized = model.normalizeSmsOutboxRecord(row, null, { strict: true });
          // eslint-disable-next-line no-await-in-loop
          normalized.id = await generateMongoId(collection, normalized.id);
  ...
  async update(id, data, options = {}) {
    return runByRepositoryBackend(options, {
      ...
      mongo: async () => {
        ...
        if (!existing) throw new Error('SMS outbox entry not found.');
        const normalized = model.normalizeSmsOutboxRecord(data, existing);
        normalized.id = existing.id;
        await collection.updateOne(resolveMongoIdFilter(id), { $set: normalized });
        return normalized;
      }
    }, 'core.smsOutbox.update');
  },
  ...
  async remove(id, options = {}) {
    return runByRepositoryBackend(options, {
      ...
      mongo: async () => {
        ...
        if (!result.deletedCount) throw new Error('SMS outbox entry not found.');
        return true;
      }
    }, 'core.smsOutbox.remove');
  }
};

assertQueryableCrudRepository('smsOutboxRepository', smsOutboxRepository);

module.exports = smsOutboxRepository;
