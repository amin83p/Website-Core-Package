...
const activityQuotaCreditGroupModel = require('../models/activityQuotaCreditGroupModel');
...
async function generateMongoGroupId(collection, requestedId = null, isoDateTime = null) {
...
async function listMongoRows(options = {}) {
  const collection = getMongoCollection('activityQuotaCreditGroups');
...
const activityQuotaCreditGroupRepository = {
  async list(options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => {
        const allRows = await activityQuotaCreditGroupModel.getAllGroups();
    ...
    }, 'core.activityQuotaCreditGroups.list');
  },
  ...
  async getById(id, options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => activityQuotaCreditGroupModel.getGroupById(id),
      mongo: async () => normalizeMongoDocument(await getMongoCollection('activityQuotaCreditGroups').findOne(resolveMongoIdFilter(id)))
    }, 'core.activityQuotaCreditGroups.getById');
  },

  async create(data, options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => activityQuotaCreditGroupModel.addGroup(data),
      mongo: async () => {
        const collection = getMongoCollection('activityQuotaCreditGroups');
        const payload = { ...(data || {}) };
        payload.id = await generateMongoGroupId(collection, payload.id, payload.dateTime);
        await collection.insertOne(payload);
        return normalizeMongoDocument(payload);
      }
    }, 'core.activityQuotaCreditGroups.create');
  },

  async update(id, data, options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => activityQuotaCreditGroupModel.updateGroup(id, data),
      mongo: async () => {
        const collection = getMongoCollection('activityQuotaCreditGroups');
        const existing = await collection.findOne(resolveMongoIdFilter(id));
        if (!existing) throw new Error('Activity quota credit group not found.');
    ...
    }, 'core.activityQuotaCreditGroups.update');
  },

  async remove(id, options = {}) {
    return runByRepositoryBackend(options, {
      json: async () => activityQuotaCreditGroupModel.deleteGroup(id),
      mongo: async () => getMongoCollection('activityQuotaCreditGroups').deleteOne(resolveMongoIdFilter(id))
    }, 'core.activityQuotaCreditGroups.remove');
  }
};

assertQueryableCrudRepository('activityQuotaCreditGroupRepository', activityQuotaCreditGroupRepository);

module.exports = activityQuotaCreditGroupRepository;
